...
vec3 camPath( float ltime )
{
    vec2 p = 600.0*vec2( cos(1.4+0.37*ltime),
                         cos(3.2+0.31*ltime) );
...
void main(void)
{
    ...
    float ltime = time*.15 + 4.0*mouse.x/resolution.x;

	vec3 light1 = normalize( vec3(  0.4, 0.22,  0.6 ) );


	vec3 ro = camPath( ltime );
	vec3 ta = camPath( ltime + 3.0 );
	ro.y = terrain3( ro.xz ) + 11.0;
	ta.y = ro.y - 100.0;

	float cr = 0.2*cos(0.1*ltime);
